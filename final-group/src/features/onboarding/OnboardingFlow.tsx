...
export function normalizeJoinCode(value: string): string { return value.trim().replace(/[\s-]+/g, "").toUpperCase(); }
...
export function OnboardingFlow({ backend, profile, onTripReady }: { backend: OnboardingBackend; profile: UserRecord; onTripReady: (trip: TripRecord) => void }) {
  ...
  async function onJoinSubmit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (submitting) return; setErrors({}); setRequestError(""); setSuccess(""); setSubmitting(true); try { const trip = await joinTrip(backend, profile, joinCode); setSuccess("Trip joined."); onTripReady(trip); } catch (error) { if (error instanceof OnboardingInputError) setErrors(error.errors); else setRequestError(error instanceof Error ? error.message : "Could not join this trip."); } finally { setSubmitting(false); } }
  return <section aria-labelledby="onboarding-title" className={styles.workbench} data-motion="calm" data-testid="onboarding-workbench" data-ui-system="light-workbench"><div aria-hidden="true" className={styles.grid} /><article className={styles.card}>
    ...
    </form></section><section aria-label="Join by code" className={styles.lockedJoin}><form aria-busy={submitting} onSubmit={onJoinSubmit}><p className={styles.lockedKicker}>JOIN BY CODE / VERIFIED</p><h2>Join a team workspace</h2><p>The code is verified against a time-limited Firestore join proof. It can only add your own account as a member.</p><label className={styles.field}><span>Join code</span><input aria-describedby={errors.joinCode ? "join-code-error" : undefined} aria-invalid={Boolean(errors.joinCode)} disabled={submitting} maxLength={24} onChange={(event) => setJoinCode(event.target.value)} placeholder="16-character code" value={joinCode} />{errors.joinCode ? <span className={styles.fieldError} id="join-code-error" role="alert">{errors.joinCode}</span> : null}</label><button className={styles.lockedAction} disabled={submitting} type="submit">{submitting ? "Joining tripâ€¦" : "Join trip"}</button></form></section></div>

...
describe("TripFlow App composition",()=>{
 ...
 it("labels the synthetic preview",async()=>{await render(<App backend={backend(user,snapshot.trip,[snapshot.trip],snapshot)} demoMode/>);expect(screen.getByTestId("local-demo-notice").textContent).toContain("Local demo data");});
...
const snapshot:TripSnapshot={trip:{id:"trip-1",name:"Da Lat",destination:"Da Lat",startDate:"2026-08-01",endDate:"2026-08-03",leadId:"user-1",joinCode:"DALAT26"},members:[{uid:"user-1",displayName:"Lan",email:"user@example.com",role:"lead",responsibility:"Timeline",isDemo:false}],events:[{id:"event-1",order:0,title:"Check in",category:"stay",startAt:"2026-08-01T08:00:00.000Z",endAt:"2026-08-01T09:00:00.000Z",status:"happening",participantIds:["user-1"],createdBy:"user-1",approvedBy:"user-1"}],expenses:[{id:"expense-1",title:"Hotel",amount:1000000,paidBy:"user-1",splitAmong:["user-1"],status:"pending",createdBy:"user-1"}]};
